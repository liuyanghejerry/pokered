...
- [x] **M8.2** 实现原版存档导入
- [x] **M8.3** 实现网络对战 (替代串口通信) ✅ — link/模块(5个文件): mod.rs(LinkState枚举+is_connected/is_battling/is_trading)、protocol.rs(LinkAction枚举+to_wire_byte/from_wire_byte匹配原版协议0x0D-0x0F+PartyExchangeData+NetworkMessage枚举含Hello/HelloAck/RequestBattle/AcceptBattle/DeclineBattle/PartyData/TurnAction/Disconnect)、transport.rs(TransportError+NetworkTransport trait+ChannelTransport基于mpsc的测试用传输层)、link_battle.rs(LinkBattleManager状态机: Idle→WaitingForHelloAck→Connected→ExchangingParties→Battling→Finished, 含版本校验/握手/战斗请求/队伍交换/回合动作交换/断开连接/重置)、link_battle_tests.rs(16个测试); lib.rs注册pub mod link; 16个新测试，总计2140测试通过
- [ ] **M8.4** 实现网络交换 ✅ — protocol.rs新增7个NetworkMessage变体(RequestTrade/AcceptTrade/DeclineTrade/SelectMon/ConfirmTrade/CancelTrade/TradeComplete); link_trade.rs(LinkTradeManager状态机: Idle→WaitingForTradeResponse/PeerRequestedTrade→SelectingMon→WaitingForPeerSelection→BothSelected→WaitingForPeerConfirm/PeerConfirmedWaitingLocal→Trading→Completed, 含请求/接受/拒绝/选择宝可梦/确认/取消/重选/断开连接/重置); link_trade_tests.rs(16个测试: 请求接受流程/拒绝流程/非法状态错误/选择宝可梦/双方确认交换执行/取消返回选择/断开连接/频道断开/空轮询/重置/意外消息错误/重新选择); mod.rs注册link_trade模块; 总计2156测试通过
...
- [x] **M9.5** WebAssembly 构建 ✅ — pokered-web crate: WASM兼容入口(305行)，PixelsBuilder::build_async()异步GPU初始化，Rgba8Unorm纹理格式(浏览器兼容)，DOM canvas自动挂载+浏览器resize监听，wasm-bindgen/web-sys集成，getrandom js feature；pokered-renderer::window模块cfg gate排除wasm32；run-wasm辅助crate(cargo-run-wasm开发服务器)；原生构建+WASM交叉编译均通过，2,452测试全部通过
